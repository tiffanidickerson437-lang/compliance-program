...
each control's named system of record (see ci_mapping in the control library)
and computes whether the control drifted. Until those endpoints are wired, the
check reads a drift-signals file holding the computed result, enriches each
entry with the owning function and framework impact from the control library,
and renders the GitHub Issue that the daily Action opens.
...
def render_issue(entry: dict, control: dict) -> str:
    ...
    lines = []
    lines.append("Title: Control drift: {} ({})".format(cid, drift_type))
...
def main(argv: list[str]) -> int:
    ...
    if not signals_path.exists():
        print("No drift-signals file at {}.".format(signals_path))
    ...
    print("Signals: {}".format(signals_path))
